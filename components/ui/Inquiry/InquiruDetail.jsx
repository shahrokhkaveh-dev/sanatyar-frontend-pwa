...
export default function InquiruDetail({ data, myproduct, t, locale }) {
    return (
        <div className="pb-2">
            ...
            <Image quality={100} className="w-full h-64 object-cover p-12 bg-white" alt="product_image" src={`${process.env.NEXT_PUBLIC_BASE_IMAGE}${data.product_image}`} width={800} height={300} />
            ...
            <div className="bg-green-100 border-[1px] border-blue-900 text-sm font-bold px-2 py-3.5 w-[95%] mx-auto rounded-lg flex flex-col gap-y-3 mt-1">
                <p className={`${data.status == 2 ? "text-green-700" : data.status == 3 ? 'text-red-600' : "text-blue-900"}`}>{t.request_status}: {t[data.status]}</p>
            ...
            {myproduct && data.status == 1 && <Response t={t} id={data.id} />}
        </div>
    );
}