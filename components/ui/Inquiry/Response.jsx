...
export default function Response({ id }) {
    ...
    const SendHandler = async () => {
        ...
        if (res.data && res.data.flag) {
            ShowMessage("پاسخ ارسال شد", setError)
            setShow(0)
            router.refresh()
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-2 py-1.5 px-2 gap-x-2">
                <button className="text-blue-800 border-[1px] border-blue-800 flex flex-row justify-around items-center rounded-md w-full px-1.5 py-2 text-sm font-bold bg-white" onClick={() => { setShow(1), setResponse((perv) => ({ ...perv, status: 2 })) }}><HiDocumentCheck className="text-blue-900 text-base" />پذیرش درخواست</button>
                <button className="text-orange-400 bg-white border-[1px] border-blue-800 flex flex-row justify-around items-center rounded-md w-full px-1.5 py-0.5 text-sm font-bold" onClick={() => { setShow(2), setResponse((perv) => ({ ...perv, status: 3 })) }} ><RiCloseLargeLine className="text-orange-400 text-base " />رد درخواست</button>
            </div>
            {show == 1 &&
                <Accept data={response} setShow={setShow} action={SendHandler} setData={setResponse} />
            }
            {show == 2 &&
                <div className="fixed top-0 right-0 w-full h-full backdrop-brightness-50 flex justify-center items-center z-40">
                    <div className="w-5/6 min-h-32 flex flex-col justify-between py-2 px-1 bg-white rounded-lg">
                        <Textarea bg={"white"} data={response} setData={setResponse} name={"response_description"} title={"توصحات رد "} />
