...
import FileInput from "../ui/AutomationSystem/FileInput";
import Input from "../ui/AutomationSystem/Input";
import Textarea from "../ui/AutomationSystem/TextArea";
...
export default function Send({ data, Fainal, locale, t }) {
    ...
    const SendHandler = async () => {
        ...
        if (res.data && res.data.flag) {
            ShowMessage(t.letter_sent_successfully, setMessage)
    ...
    return (
        <div className="min-h-screen">
            ...
            <HeaderItems href={`/${locale}/AutomationSystem`} title={t.send_title} />
            <div className="mt-2.5 flex flex-col gap-y-2 px-2">
                <Input disable={searchParams.get('company') ? true : false} setinitdata={setLetter} setError={setError} data={company} setData={setCompany} bg={"white"} error={error} name={'name'} title={t.receiver} />
                <div>
                ...
                <Input setError={setError} data={Letter} setData={setLetter} bg={"white"} error={error} name={'subject'} title={t.subject} />
                <Textarea bg={"white"} data={Letter} setData={setLetter} title={t.write_text} name={"content"} />
                <FileInput data={Letter} setData={setLetter} name={"attachment"} t={t} />
            </div>
            <div className="px-2">
                <button onClick={SendHandler} className="w-full py-4 text-base rounded-lg bg-blue-800 text-white mt-2">{t.send_letter}</button>
            </div>
        </div>
    );
}