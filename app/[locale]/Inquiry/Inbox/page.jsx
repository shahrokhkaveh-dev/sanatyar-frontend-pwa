...
export default async function page({ params }) {
    ...
    const res = await api.get('application/panel/products/request').catch(err => servError(err))
            ...
            <div className="flex flex-col gap-y-2 py-2 px-1.5 ">
                {res.data.response.data.length > 0 && res.data.response.data.map((i) => (
                    <div key={i.id} className={`border-[1px] flex flex-col border-blue-900  rounded-md ${i.status === 1 ? "bg-white" : i.status === 2 ? "bg-green-100" : i.status === 3 ? "bg-red-100" : "bg-white"} `}>
                        ...
                        <div className="flex flex-row justify-between items-center px-2 py-1.5 text-[10px] ">
                            <p className=""> {t.request_status}: {t[i.status]}</p>
                        ...
                        <div className="border-t-[1px] border-black p-1 flex justify-center">
                            <Link href={`/${locale}/Inquiry/Inbox/${i.id}`} className="bg-blue-800 text-white text-base font-bold py-2 px-3 rounded-md">{t.show_inquiry_details}</Link>
